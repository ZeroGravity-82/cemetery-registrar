...
SELECT b.id                                                                              AS id,
       ...
       IF(
           dnp.deceased_details->>"$.deathCertificate" IS NOT NULL,
           CONCAT(
               dnp.deceased_details->>"$.deathCertificate.series",
               ' № ',
               dnp.deceased_details->>"$.deathCertificate.number",
               ' от ',
               DATE_FORMAT(dnp.deceased_details->>"$.deathCertificate.issuedAt", '%d.%m.%Y')
           ),
           NULL
       )                                                                                 AS deceasedNaturalPersonDeceasedDetailsDeathCertificateComposed,
       IF(
           dnp.deceased_details->>"$.cremationCertificate" IS NOT NULL,
           CONCAT(
               '№ ',
               dnp.deceased_details->>"$.cremationCertificate.number",
               ' от ',
               DATE_FORMAT(dnp.deceased_details->>"$.cremationCertificate.issuedAt", '%d.%m.%Y')
           ),
           NULL
       )                                                                                 AS deceasedNaturalPersonDeceasedDetailsCremationCertificateComposed,
       ...
       IF(
          cnp.passport IS NOT NULL,
          CONCAT(
             cnp.passport->>"$.series",
             ' № ',
             cnp.passport->>"$.number",
             ', выдан ',
             cnp.passport->>"$.issuedBy",
             ' ',
             DATE_FORMAT(cnp.passport->>"$.issuedAt", '%d.%m.%Y'),
             IF(
                cnp.passport->>"$.divisionCode" <> 'null',
                CONCAT(' (', cnp.passport->>"$.divisionCode", ')'),
                ''
             )
          ),
          NULL
       )                                                                                 AS customerNaturalPersonPassportComposed,
       ...
       IF(
          bpgspic.passport IS NOT NULL,
          CONCAT(
             bpgspic.passport->>"$.series",
             ' № ',
             bpgspic.passport->>"$.number",
             ', выдан ',
             bpgspic.passport->>"$.issuedBy",
             ' ',
             DATE_FORMAT(bpgspic.passport->>"$.issuedAt", '%d.%m.%Y'),
             IF(
                bpgspic.passport->>"$.divisionCode" <> 'null',
                CONCAT(' (', bpgspic.passport->>"$.divisionCode", ')'),
                ''
             )
          ),
          NULL
       )                                                                                 AS graveSitePersonInChargePassportComposed,
       ...
       IF(
          bpcnpic.passport IS NOT NULL,
          CONCAT(
             bpcnpic.passport->>"$.series",
             ' № ',
             bpcnpic.passport->>"$.number",
             ', выдан ',
             bpcnpic.passport->>"$.issuedBy",
             ' ',
             DATE_FORMAT(bpcnpic.passport->>"$.issuedAt", '%d.%m.%Y'),
             IF(
                bpcnpic.passport->>"$.divisionCode" <> 'null',
                CONCAT(' (', bpcnpic.passport->>"$.divisionCode", ')'),
                ''
             )
          ),
          NULL
       )                                                                                 AS columbariumNichePersonInChargePassportComposed,
       ...
       IF(
          bpmtpic.passport IS NOT NULL,
          CONCAT(
             bpmtpic.passport->>"$.series",
             ' № ',
             bpmtpic.passport->>"$.number",
             ', выдан ',
             bpmtpic.passport->>"$.issuedBy",
             ' ',
             DATE_FORMAT(bpmtpic.passport->>"$.issuedAt", '%d.%m.%Y'),
             IF(
                bpmtpic.passport->>"$.divisionCode" <> 'null',
                CONCAT(' (', bpmtpic.passport->>"$.divisionCode", ')'),
                ''
             )
          ),
          NULL
       )                                                                                 AS memorialTreePersonInChargePassportComposed,
...
LIKE_TERM_SQL;
    ...
    private function hydrateView(array $viewData): BurialView
    {
        return new BurialView(
            ...
            $viewData['deceasedNaturalPersonDeceasedDetailsDeathCertificateComposed'],
            $viewData['deceasedNaturalPersonDeceasedDetailsCremationCertificateComposed'],
            $viewData['customerId'],
            ...
            $viewData['customerNaturalPersonPassportComposed'],
            ...
            $viewData['graveSitePersonInChargePassportComposed'],
            ...
            $viewData['columbariumNichePersonInChargePassportComposed'],
            ...
            $viewData['memorialTreePersonInChargePassportComposed'],
